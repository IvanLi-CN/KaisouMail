...
- Cloudflare's REST API currently allows `1,200 requests / 5 minutes / API token`, so orphaned subdomain DNS cleanup now runs through a minute dispatcher plus queue consumer pipeline: the dispatcher only leases/enqueues orphaned hosts that still have a zone id, the queue consumer batches up to `4` hosts at a time so same-zone Email Routing unlocks can be deduplicated within the batch, queued work is dropped safely if the domain loses its zone id before consume, the cleanup kill switch (`EMAIL_ROUTING_MANAGEMENT_ENABLED=false` or `SUBDOMAIN_CLEANUP_BATCH_SIZE=0`) stops both new dispatch and already-enqueued host work immediately, a shared D1-backed request gate releases exactly `4 requests / second` globally **for subdomain cleanup traffic only**, `429` retries are deferred to the next minute instead of writing row backoff, auth failures pause new dispatches briefly, and `SUBDOMAIN_CLEANUP_BATCH_SIZE=50` still only defines the candidate scan window
