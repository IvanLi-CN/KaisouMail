...
const requiredEnv = ["VERSION_CODE", "VERSION_FILE", "EXPECTED_SHA"];

for (const name of requiredEnv) {
  if (!(name in process.env) || !process.env[name]?.trim()) {
    console.error(`Missing required pages-smoke environment variable: ${name}`);
    process.exit(1);
  }
}

const smokeLabel = process.env.SMOKE_LABEL?.trim() || "Pages same-origin";
