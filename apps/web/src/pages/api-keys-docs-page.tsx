...
const buildEndpointGroups = (meta: ApiMeta): EndpointGroup[] => {
  ...
  "addressRules": {
    ...
    "examples": ${JSON.stringify(meta.addressRules.examples, null, 4)}
          ...
          notes: [
            ...
            "有限 TTL 统一按分钟表达；传 `expiresInMinutes: null` 表示永不过期，省略该字段则回退到默认 TTL。",
  ...
  "mailboxes": [
    {
      ...
      "routingRuleId": "rule_alpha"
          ...
          notes: [
            "列表响应包装在 `{ mailboxes: [...] }` 下。",
            "字段集合由 `mailboxSchema` 定义，包含 `lastReceivedAt`、`expiresAt` 与 `routingRuleId`；永不过期邮箱的 `expiresAt` 会是 null。",
  ...
  "routingRuleId": "rule_alpha"
          ...
          notes: [
            ...
            `expiresInMinutes 在有限模式下必须是 ${meta.minMailboxTtlMinutes} 到 ${maxTtl} 之间的整数；传 null 表示无限，未传时默认 ${ttl}。`,
          ],
        },
        {
          method: "POST",
  ...
  "routingRuleId": "rule_alpha"
          ...
          notes: [
            ...
            "若要创建永不过期邮箱，可显式传 `expiresInMinutes: null`。",
          ],
        },
        {
          method: "GET",
          ...
          <CardContent className="space-y-5">
            ...
            <div className="rounded-xl border border-border/70 bg-muted/20 p-4 text-sm leading-6 text-muted-foreground">
              <p>
                ...
                <code>expiresInMinutes: null</code> 表示永不过期。
              </p>
              <p className="mt-2">
