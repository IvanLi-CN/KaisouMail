...
- Mailbox presentation keeps explicit lifecycle segmentation in management views, while the workspace rail omits expired rows entirely; Catch All remains an explicit operator-facing badge, mailbox management tables render routing state as badges for per-address rules, domain-level delivery, and removed inactive rules, the workspace rail keeps right-aligned numeric badges, and mailbox tables show unread / total counts
