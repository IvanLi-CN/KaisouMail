...
- GitHub and LinuxDO OAuth use same-origin callback URLs at `/api/auth/{provider}/callback`; the admin registration settings surface shows those callback URLs with copy actions, while LinuxDO issuer overrides remain deployer-owned through an explicitly configured `LINUXDO_OAUTH_BASE_URL`, which takes precedence over any stored/default issuer value without being written back to stored registration settings by unrelated admin saves
...
- Catch-all cutover tasks never use historical DNS records or historical `subdomains` rows as a truth source. Wildcard target now means the enabled Catch All low-record DNS baseline, while explicit target is only used by disable paths and recalculates the exact-host set solely from live database mailboxes.
- Wildcard cutover failures write `wildcardDnsLastError` and fail the task without attempting an explicit DNS fallback; disable DNS rebuilds and registered-route backfill still batch across resumable task invocations so large domains can finish without tripping the single-invocation Worker subrequest ceiling.
...
- 2026-05-09: Made Catch All enable always target wildcard subdomain DNS, removed `WILDCARD_SUBDOMAIN_DNS_*` from the business decision path, and kept explicit DNS rebuilds restricted to disable flows.
