...
- Catch-all domains expose a domain-level subdomain DNS mode (`explicit | wildcard`) together with last wildcard verification / error state so rollout can target low-risk roots incrementally without hiding fallback status
- Catch-all enable/disable actions now hand off to async cutover tasks; operators can follow task state through the dedicated admin task API and an SSE event stream while long-running DNS cleanup / rebuild work progresses
...
- `GET /api/domain-cutover-tasks/:taskId` returns the current catch-all cutover task snapshot, while `GET /api/domain-cutover-tasks/:taskId/events` streams the same task as SSE progress / completed / failed events for long-running DNS reconcile operations; tasks may yield back to `pending` between DNS / route batches and must resume from the last persisted phase instead of restarting from `loading_state`
...
- `POST /api/domains/:id/catch-all/enable` now returns `202 + taskId` and acts as the async reconcile entry for the runtime-selected target mode. When `WILDCARD_SUBDOMAIN_DNS_ENABLED=true` and the domain is allowlisted, the task purges every project-created exact Email Routing `MX/TXT` host under that root, ensures wildcard `*.rootDomain` Email Routing DNS, and only then persists `catchAllEnabled=true + subdomainDnsMode=wildcard`. If runtime target mode is `explicit`, the same entry instead deletes project wildcard Email Routing DNS, purges every project-created exact Email Routing host, and rebuilds exact DNS from the current live mailbox set before persisting `catchAllEnabled=true + subdomainDnsMode=explicit`.
- Catch-all cutover tasks never use historical DNS records or historical `subdomains` rows as a truth source. Wildcard target means `exact mailbox DNS = 0`, while explicit target means the exact-host set is recalculated solely from live database mailboxes.
- Wildcard cutover failures write `wildcardDnsLastError`, fail the task, and attempt an in-task rollback back to explicit DNS derived from the current live mailbox set instead of silently leaving a half-migrated state; both purge/rebuild DNS work and registered-route backfill must batch across resumable task invocations so large domains can finish without tripping the single-invocation Worker subrequest ceiling.
- Legacy stored catch-all restore snapshots remain valid even when historical `actions[*].value` arrays were omitted; missing arrays are interpreted as `[]` rather than wedging the cutover task in `loading_state`.
...
- Wildcard cleanup only touches non-apex, non-wildcard, Email Routing `MX/TXT` records that belong to the managed mailbox root domain; project access domains, Worker custom domains, A/AAAA/CNAME/HTTPS/CAA records, non-Email-Routing TXT records, and apex Email Routing records are never part of the purge set.
- `POST /api/mailboxes` accepts optional `mailDomain`; `rootDomain` remains a compatibility alias, and when both are omitted the API randomly selects one active mailbox domain server-side, while `expiresInMinutes` can be omitted for the runtime default TTL or set to `null` for a long-term mailbox; on `catchAllEnabled=true` domains it now registers the mailbox locally without creating a per-address Cloudflare routing rule, wildcard-verified `subdomainDnsMode=wildcard` domains skip per-subdomain Email Routing DNS enables for newly seen hosts, and allowlisted catch-all domains that have not completed wildcard verification fail mailbox writes instead of recreating exact DNS as a silent fallback.
...
- `POST /api/mailboxes/ensure` accepts either `address` or `localPart + subdomain (+ optional mailDomain/rootDomain)`, reuses an existing visible `active` or `expired` mailbox when present, and otherwise creates a fresh mailbox; when `expiresInMinutes` is provided for an existing mailbox, the API only extends TTL and never shortens it, with extension restoring an `expired` mailbox to `active`; promoting a `source=catch_all` mailbox on a Catch-all-enabled domain stays a local-only state transition (`catch_all -> registered`) without an extra per-address Cloudflare rule write, wildcard-verified domains never recreate exact subdomain DNS, and allowlisted domains that are still waiting on wildcard cutover fail the ensure request instead of silently dropping back to explicit provisioning.
...
- `GET /api/mailboxes` accepts optional `scope=workspace` and `status`; the default scope returns the caller's full mailbox history, while workspace scope keeps all `active` / `destroying` rows, excludes `expired`, and only the newest 50 `destroyed` rows whose `destroyedAt` is within the last 7 days, preserving that destroyed-history slice in descending `destroyedAt` order
...
- Disabling a domain automatically turns off the project-managed Catch All first, backfills any active `registered` mailboxes that still depend on Catch-all with per-address Cloudflare routing rules, restores the previous Cloudflare catch-all rule, and only then lets the domain enter `disabled`
...
- Mailbox destroy/cleanup now converts due `active` rows into `expired`, keeps expired mailboxes recoverable for 24 hours before cleanup, keeps message metadata until R2 deletion succeeds, hides `destroying` mailbox messages from read APIs, clears `lastReceivedAt` hydration for `destroying` mailboxes so the rail stays aligned with the visible message feed, retries mailboxes stuck in `destroying` ahead of eligible expired cleanup work while continuing through the rest of the batch even after an earlier mailbox failure, and moves orphaned-subdomain cleanup onto a minute dispatcher plus queue-consumer pipeline: the dispatcher only leases and enqueues orphaned rows that still have a known `zoneId`, the queue consumer batches up to `4` hosts so same-zone Email Routing unlocks can be deduplicated within a batch, explicit cleanup unlocks zone-level Email Routing DNS once before deleting legacy exact-host MX/SPF records, queued work is dropped safely if the domain loses its `zoneId` before consume, `EMAIL_ROUTING_MANAGEMENT_ENABLED=false` or `SUBDOMAIN_CLEANUP_BATCH_SIZE=0` stops both new dispatch and already-enqueued host work immediately, a D1-backed request gate releases exactly `4 Cloudflare requests / second` for subdomain-cleanup traffic only, `SUBDOMAIN_CLEANUP_BATCH_SIZE=50` remains the candidate scan window, `SUBDOMAIN_CLEANUP_DISPATCH_BATCH_SIZE=48` caps per-minute fan-out, any upstream/local `429` retries only the current host on the next minute without row backoff, authentication failures pause fresh dispatch briefly, wildcard-born orphan rows are deleted locally without per-host Cloudflare DNS calls, explicit rows are removed only after successful exact-fqdn MX/SPF cleanup, and reused hosts still preserve cleanup markers so DNS can be re-enabled safely
...
- Mailbox presentation keeps explicit lifecycle segmentation in management views, while the workspace rail omits expired rows entirely; Catch All remains an explicit operator-facing badge, the workspace rail keeps right-aligned numeric badges, and mailbox tables show unread / total counts
...
- 2026-04-21: Replaced synchronous catch-all enable/disable with async domain cutover tasks plus task-state/SSE APIs, made wildcard cutover purge every project-created exact Email Routing mailbox host before creating wildcard DNS, made explicit rebuild ignore historical DNS by recalculating exact hosts solely from live mailboxes, and then hardened the task runner so large cutovers resume phase-by-phase in bounded DNS/route batches while legacy restore snapshots missing `actions[*].value` still parse cleanly.
...
- 2026-04-20: Replaced the single-invocation orphaned-subdomain cleanup with a minute dispatcher + queue-consumer pipeline, added D1-backed row leases plus a global `4 requests / second` Cloudflare request gate, removed inlined subdomain cleanup from mailbox cleanup, and taught cleanup retries to defer `429` / auth failures to later queue deliveries instead of burning the whole scheduled invocation.
- 2026-04-19: Added gradual orphaned subdomain DNS reclamation for Email Routing (`cleanupNextAttemptAt` / `cleanupLastError`), wired the scheduled cleanup pipeline to delete only exact-fqdn Email Routing MX/SPF records with shared Cloudflare `429` cooldown handling, skipped legacy domains that still lack a backfilled `zoneId`, then replaced the request-budget design with a fixed `1 request / second` Cloudflare pacer, a 12-minute wall-clock deadline, and a single-flight runtime lease; `SUBDOMAIN_CLEANUP_BATCH_SIZE=50` is now just the candidate scan window, any `429` aborts the current pass until the next scheduled run, and unfinished exact-fqdn cleanup resumes later without losing revive/re-enable safety.
...
PR: include
![Domains bind card blocking direct subdomain input and recommending apex plus mailbox subdomain instead](./assets/domains-subdomain-bind-blocked.png)

PR: include
![Domains apex bind follow-up dialog explaining authoritative nameserver cutover after Cloudflare allocation](./assets/domains-apex-bind-next-steps.png)

PR: include
![Domains catalog showing an externally existing zone record that can still be enabled without using direct bind](./assets/domains-existing-child-zone-catalog-enable.png)
