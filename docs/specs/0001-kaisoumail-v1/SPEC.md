...
- Catch-all domains now expose a domain-level subdomain DNS mode (`explicit | wildcard`) together with last wildcard verification / error state so rollout can target low-risk roots incrementally without hiding fallback status
...
- `POST /api/domains/:id/catch-all/enable` now acts as the atomic wildcard cutover entry for active catch-all domains that are included in `WILDCARD_SUBDOMAIN_DNS_ALLOWLIST` while `WILDCARD_SUBDOMAIN_DNS_ENABLED=true`: the API clones the apex Email Routing DNS template through Cloudflare's generic DNS CRUD to ensure `*.rootDomain` Email Routing `MX` / SPF `TXT` records without hardcoding provider targets, and it only persists `catchAllEnabled=true + subdomainDnsMode=wildcard` after that ensure succeeds. If the wildcard ensure fails, the endpoint returns a non-2xx error, persists `wildcardDnsLastError`, and leaves the domain on its previous state instead of silently falling back to `explicit`. Disable remains the explicit rollback path and always returns the domain to `explicit`.
- Wildcard rollout is limited to `catchAllEnabled=true` domains; non-catch-all domains always stay on explicit per-subdomain DNS in V1. Historical exact records may coexist with the wildcard because specific host records take precedence over wildcard DNS, so cleanup can retire legacy exact hosts gradually.
