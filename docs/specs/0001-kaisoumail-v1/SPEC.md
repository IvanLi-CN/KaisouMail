...
- `GET /api/mailboxes` accepts optional `scope=workspace`; the default scope returns the caller's full mailbox history, while workspace scope keeps all `active` / `destroying` rows and only the newest 50 `destroyed` rows whose `destroyedAt` is within the last 7 days
...
- Mailbox destroy/cleanup now keeps message metadata until R2 deletion succeeds, hides `destroying` mailbox messages from read APIs, and scheduled cleanup retries mailboxes stuck in `destroying` ahead of fresh `active` cleanup work while continuing through the rest of the batch even after an earlier mailbox failure
