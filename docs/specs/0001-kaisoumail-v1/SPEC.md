...
- Catch-all domains expose a domain-level subdomain DNS mode (`explicit | wildcard`) together with last wildcard verification / error state so rollout can target low-risk roots incrementally without hiding fallback status; wildcard mode is now the low-record DNS baseline, while fresh active subdomains still go through explicit Cloudflare Email Routing onboarding at the subdomain scope before the project re-purges the exact MX/TXT it just created
...
- Catch-all cutover tasks never use historical DNS records or historical `subdomains` rows as a truth source. Wildcard target now means a low-record DNS baseline, while explicit target means the exact-host set is recalculated solely from live database mailboxes.
...
- `GET /api/messages` accepts repeated `mailbox` params, optional repeated `mailboxId` params, optional repeated `mailboxStatus` params, plus `after` / `since` ISO datetime filters and optional `scope=workspace`; when both cursor aliases are present, the later timestamp is used as the strict lower bound, workspace scope can pin reused-address mailbox selection by `mailboxId`, and trash aggregation uses `mailboxStatus=expired` so clients do not have to serialize thousands of mailbox IDs into one URL
