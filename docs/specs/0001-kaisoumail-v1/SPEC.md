...
- GitHub and LinuxDO OAuth use same-origin callback URLs at `/api/auth/{provider}/callback`; the admin registration settings surface shows those callback URLs with copy actions, while LinuxDO issuer overrides remain deployer-owned through `LINUXDO_OAUTH_BASE_URL` and take precedence over any stored/default issuer value
