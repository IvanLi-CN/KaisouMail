...
- `GET /api/domain-cutover-tasks/:taskId` returns the current catch-all cutover task snapshot, while `GET /api/domain-cutover-tasks/:taskId/events` streams the same task as SSE progress / completed / failed events for long-running DNS reconcile operations
...
- Wildcard cutover failures write `wildcardDnsLastError`, fail the task, and attempt an in-task rollback back to explicit DNS derived from the current live mailbox set instead of silently leaving a half-migrated state.
...
- 2026-04-21: Replaced synchronous catch-all enable/disable with async domain cutover tasks plus task-state/SSE APIs, made wildcard cutover purge every project-created exact Email Routing mailbox host before creating wildcard DNS, and made explicit rebuild ignore historical DNS by recalculating exact hosts solely from live mailboxes.
