...
- Mailbox TTL entry now uses a non-linear slider plus inline editable display: finite values span `1 hour .. 30 days` on a logarithmic scale, while an explicit final slot and `expiresInMinutes: null` represent unlimited lifetime
...
- `POST /api/mailboxes` accepts optional `rootDomain`; when omitted, the API randomly selects one active mailbox domain server-side, while `expiresInMinutes` can be omitted for the runtime default TTL or set to `null` for an unlimited mailbox
...
- Mailbox lifetime control now keeps the dense dialog compact: a log-scale slider covers short-lived to long-lived finite mailboxes, the current resolved TTL is shown inline at the right edge, double-click inline editing accepts common human units, and the terminal slider slot exposes explicit unlimited lifetime
...
- 2026-04-10: Replaced the mailbox TTL number input with a logarithmic `1 hour .. 30 days + unlimited` slider plus double-click inline duration editing, split runtime semantics into `rootDomain omitted => random` and `expiresInMinutes omitted => default / null => unlimited`, and updated API/runtime surfaces so unlimited mailboxes persist with `expiresAt = null`.
...
PR: include
![Mailbox create card with the finite TTL slider pushed to the 30-day cap](./assets/mailbox-create-ttl-max-finite.png)

PR: include
![Mailbox create card with the explicit unlimited TTL slot selected](./assets/mailbox-create-ttl-unlimited.png)
