...
- Mailbox destroy/cleanup now keeps message metadata until R2 deletion succeeds, hides `destroying` mailbox messages from read APIs, clears `lastReceivedAt` hydration for `destroying` mailboxes so the rail stays aligned with the visible message feed, retries mailboxes stuck in `destroying` ahead of fresh `active` cleanup work while continuing through the rest of the batch even after an earlier mailbox failure, and then runs a second orphaned-subdomain pass that deletes only exact-fqdn Email Routing `MX` / SPF `TXT` records through Cloudflare DNS-record deletes; that pass is strictly serial, only enqueues domains that still have a known `zoneId`, defaults to a two-layer backlog drain budget (`SUBDOMAIN_CLEANUP_BATCH_SIZE=200` hosts scanned and `SUBDOMAIN_CLEANUP_REQUEST_BUDGET=400` as a hard Cloudflare-request ceiling per scheduled run, with a minimum valid request budget of `4` so each pass can still make forward progress), also re-selects rows that still carry cleanup markers even after a mailbox becomes live again, stops mid-host when the remaining request budget is exhausted, records Cloudflare requests even when a later delete fails, leaves partial exact-fqdn cleanup pending for a later pass (while keeping the row marked so reused subdomains re-enable DNS), honors the shared Cloudflare cooldown on upstream/local `429`, writes a per-row `cleanupNextAttemptAt + cleanupLastError` one-hour backoff on non-429 failures, deletes the local `subdomains` row only after successful DNS cleanup, and applies the same explicit subdomain lifecycle to Catch All domains
...
- 2026-04-19: Added gradual orphaned subdomain DNS reclamation for Email Routing (`cleanupNextAttemptAt` / `cleanupLastError`), wired the scheduled cleanup pipeline to delete only exact-fqdn Email Routing MX/SPF records with shared Cloudflare `429` cooldown handling, skipped legacy domains that still lack a backfilled `zoneId`, then raised the default backlog drain design to `SUBDOMAIN_CLEANUP_BATCH_SIZE=200` plus `SUBDOMAIN_CLEANUP_REQUEST_BUDGET=400`; the request budget is now enforced as a true per-run hard cap even within one host, failed Cloudflare deletes still debit that budget, and unfinished exact-fqdn cleanup resumes on later scheduled passes so production cleanup converges within hours instead of days while keeping the same shared cooldown guardrail.
