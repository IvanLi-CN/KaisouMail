...
- 2026-04-16: Expanded verification-code detection to recognize traditional-Chinese security-code copy and bounded short single-hyphen verification tokens (for cases like `WXN-DTJ`), kept pure-alpha subject matches limited to explicit `... confirmation code [is] WXN-DTJ`-style assignment cues, scoped email-validation phrasing so it only helps standalone nearby hyphenated verification blocks that are still introduced as a `code`, preserved those standalone block detections even when the surrounding copy mentions the destination email address, restored boundary checks for glued `code1234`-style text, added label-aware scoring so nearby reference/order IDs do not outrank the actual OTP, and kept host fragments, email-address fragments, long identifiers, generic all-caps hyphenated words, and promotional “use code” messages out of the verification path.
