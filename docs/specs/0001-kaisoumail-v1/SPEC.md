...
- 2026-04-16: Expanded verification-code detection to recognize traditional-Chinese security-code copy and bounded single-hyphen verification tokens (for cases like `WXN-DTJ`), scoped email-validation phrasing so it only helps nearby hyphenated verification blocks instead of unrelated numeric references, restored boundary checks for glued `code1234`-style text, and kept host fragments, long identifiers, and promotional “use code” messages out of the verification path.
