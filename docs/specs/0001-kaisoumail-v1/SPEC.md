...
- Catch-all domains expose a domain-level subdomain DNS mode (`explicit | wildcard`) together with last wildcard verification / error state; enabling Catch All always requires wildcard mode as the low-record DNS baseline, while fresh active subdomains still go through explicit Cloudflare Email Routing onboarding at the subdomain scope before the project re-purges the exact MX/TXT it just created
...
- `POST /api/domains/:id/catch-all/enable` now returns `202 + taskId` and always reconciles to wildcard mode: the task purges every project-created exact Email Routing `MX/TXT` host under that root, ensures wildcard `*.rootDomain` Email Routing DNS, and only then persists `catchAllEnabled=true + subdomainDnsMode=wildcard`. If wildcard DNS has conflicting non-matching `*.rootDomain` MX/TXT records or Cloudflare rejects the write, the task fails, records `wildcardDnsLastError`, and does not fall back to explicit subdomain DNS.
- Catch-all cutover tasks never use historical DNS records or historical `subdomains` rows as a truth source. Wildcard target now means the enabled Catch All low-record DNS baseline, while explicit target is only used by disable/rollback paths and recalculates the exact-host set solely from live database mailboxes.
...
- 2026-05-09: Made Catch All enable always target wildcard subdomain DNS, removed `WILDCARD_SUBDOMAIN_DNS_*` from the business decision path, and kept explicit DNS rebuilds restricted to disable / rollback flows.
