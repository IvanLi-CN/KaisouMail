...
- `POST /api/mailboxes/ensure` accepts either `address` or `localPart + subdomain (+ optional rootDomain)`, reuses an existing visible `active` mailbox when present, and otherwise creates a fresh mailbox; promoting a `source=catch_all` mailbox on a Catch-all-enabled domain is now a local-only state transition (`catch_all -> registered`) without an extra Cloudflare rule write
