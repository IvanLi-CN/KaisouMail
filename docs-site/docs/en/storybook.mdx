---
title: Storybook
description: Redirect to the CF Mail Storybook surface.
---

import { useEffect, useMemo } from "react";

export const DEFAULT_LOCAL_STORYBOOK_DEV_ORIGIN = "http://127.0.0.1:30031";
...
export function getLocalStorybookDevOrigin(currentUrl) {
  return process.env.RSPRESS_STORYBOOK_DEV_ORIGIN || `${currentUrl.protocol}//${currentUrl.hostname}:30031`;
}
...
export const StorybookRedirect = () => {
  ...
  return (
    <>
      ...
      <p>
        If the redirect does not happen automatically, open <a href={fallbackHref}>CF Mail Storybook</a>.
      </p>
    </>
  );
};
