...
- Catch-all support does **not require any new secret names**. Runtime still
  uses `CLOUDFLARE_RUNTIME_API_TOKEN`, with `CLOUDFLARE_API_TOKEN` as the
  shared-token fallback.
- Cloudflare's official API docs state:
  - `GET /zones/:zone_id/email/routing/rules/catch_all` accepts
    `Email Routing Rules Read` or `Email Routing Rules Write`
  - `PUT /zones/:zone_id/email/routing/rules/catch_all` requires
    `Email Routing Rules Write`
- KaisouMail already requires `Zone: Email Routing Rules: Edit`, so the
