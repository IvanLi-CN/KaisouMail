...
### 步骤 1：在 `/domains` 中填写 apex 根域名

打开控制台 `/domains`，在“绑定新域名”卡片里输入根域名：

![在 /domains 中填写根域名并准备绑定到 Cloudflare](./assets/project-domain-binding-ui-bind.png)

### `/api/domains/bind` 只支持 apex {#bind-apex-only}

如果你想得到 `user@mail.customer.com` 这类地址，不要把 `mail.customer.com`
直接提交给 `/api/domains/bind`。当前产品的推荐路径是：

1. 先绑定 apex：`customer.com`
2. 创建邮箱时把 `subdomain` 设成 `mail`
3. 最终地址仍然会是 `user@mail.customer.com`

如果 Cloudflare 里已经存在某个 zone，可以改走
[`/domains` catalog 启用](/zh/domain-catalog-enablement)；但当前产品不把
...
### 步骤 3：如果页面没有直接变成 `active`，先去注册商修改 apex 的权威 NS
...
如果你没有先完成 apex 的权威 NS 切换，就反复点“重试接入”，通常不会成功。
...
这说明 Cloudflare 已经接受了 apex zone 创建请求，但该域名还没有完成权威 NS 切换，所以 Email Routing 暂时无法启用。
...
2. 点击该行操作列里的 **详情图标**，把弹窗里的 Cloudflare nameserver 改到 apex 域名的注册商 / DNS 提供商后台
3. 等 zone 变成 `active`
