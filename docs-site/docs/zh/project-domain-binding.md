...
这时要看同一行的 **`ZONE / NS`** 列，把 Cloudflare 分配的 nameserver 原样抄到你的域名注册商后台。
...
在控制台里，这类域名通常会保留在列表中，并显示 `provisioning_error`，同时在 `ZONE / NS` 列给出 Cloudflare nameserver，并提供“重试接入”按钮：
...
2. 把注册商上的 nameserver 改成该行 `ZONE / NS` 列里 Cloudflare 提供的值
3. 等 zone 变成 `active`
